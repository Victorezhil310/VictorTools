  ...
  return (
    <div className="space-y-6 max-w-xl mx-auto">
      <div className="text-center pb-2">
        <h2 className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Convert PDF to Word online</h2>
      </div>
        ...
        {files.length > 0 && !downloadUrl && !errorMsg && (
          ...
          >
            {isProcessing ? (
              ...
              <>
                <FileText className="h-4.5 w-4.5" /> Convert PDF to Word
              </>
            )}
          </button>
        )}
        ...
        {downloadUrl && (
          <div className="w-full space-y-3">
            <div className="flex items-center gap-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-3 text-xs text-emerald-500 font-semibold justify-center">
              <CheckCircle className="h-4.5 w-4.5" /> PDF converted to editable DOCX!
            </div>

            <a
              href={downloadUrl}
              download={`${files[0]?.name.replace(/\.[^/.]+$/, "") || "converted"}.docx`}
            ...
            >
              <Download className="h-4.5 w-4.5" /> Download Word Document
            </a>
